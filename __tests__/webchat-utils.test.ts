...
describe('getRegionalSettingsURL', () => {
  ...
  it('throws on missing api-version', () => { const bad = 'https://domain/powervirtualagents/.../token'; expect(() => getRegionalSettingsURL(bad)).toThrowError() })
  it('throws on invalid endpoint', () => { const bad = 'https://domain/not-pva'; expect(() => getRegionalSettingsURL(bad)).toThrowError() })
})
