...
async function tryIssue(url: string, headers: Record<string,string>){
  for (const method of ['GET','POST'] as const){
    ...
    if (res.ok && json && json.token){ return { ok: true, token: json.token } }
    if (method === 'GET' && (res.status === 401 || res.status === 405)) continue
    if (method === 'POST' && res.status === 405) continue
    if (method === 'POST'){ return { ok: false, status: res.status, body: text } }
  }
...
export async function GET(){
  ...
  try{
    if(tenant && clientId && clientSecret){
      ...
      const aad = await fetch(`https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`, { method:'POST', headers:{ 'Content-Type':'application/x-www-form-urlencoded' }, body: form, cache: 'no-store' })
